SMODS.Joker{ --Troll Joker
    ...
    loc_txt = {
        ...
        ['text'] = {
            ...
            [2] = 'if played hand contains a {C:attention}Flush{}',
    ...
    calculate = function(self, card, context)
        if context.cardarea == G.jokers and context.joker_main  then
            if (next(context.poker_hands["Flush"]) and G.GAME.blind:get_type() == 'Small') then
                return {
                    mult = G.GAME.round_resets.ante
                }
            elseif (next(context.poker_hands["Flush"]) and G.GAME.blind:get_type() == 'Big') then
                return {
                    mult = (G.GAME.round_resets.ante) * 3
                }
            elseif (next(context.poker_hands["Flush"]) and G.GAME.blind.boss) then
                return {
