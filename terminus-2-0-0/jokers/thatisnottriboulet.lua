SMODS.Joker{ --That Is Not Triboulet!
    ...
    loc_txt = {
        ...
        ['text'] = {
            ...
            [2] = 'when a King or Queen is scored',
            [3] = '{C:inactive}(Currently: {}{C:enhanced,s:1.2}#1#{}{C:inactive}/69 Unfunity Points){}',
    ...
    calculate = function(self, card, context)
        if context.selling_self  then
            if ((card.ability.extra.unfunity or 0) >= 69 and (card.ability.extra.unfunity or 0) < 138) then
                return {
                    func = function()
            ...
            elseif (card.ability.extra.unfunity or 0) >= 138 then
                return {
                    func = function()
            local created_joker = true
            G.E_MANAGER:add_event(Event({
                func = function()
                    local joker_card = SMODS.add_card({ set = 'Joker', key = 'j_teu_joanreneu' })
                    if joker_card then
